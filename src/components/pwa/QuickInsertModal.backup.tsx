    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-6">
        <motion.div
          ...
          className={`w-full max-w-md md:max-w-lg bg-gradient-to-b ${
            type === "income"
              ? "from-blue-950/20 via-zinc-950 to-black border-t border-blue-500/20 md:border md:border-blue-500/20 shadow-blue-500/5"
              : "from-red-950/20 via-zinc-950 to-black border-t border-red-500/20 md:border md:border-red-500/20 shadow-red-500/5"
          } rounded-t-[32px] md:rounded-3xl flex flex-col overflow-hidden md:overflow-visible shadow-2xl`}
        ...
        >
          ...
          <div className="flex justify-end px-6 pt-6 pb-2">
            <button
              type="button"
              onClick={onClose}
              className="p-2 text-zinc-500 hover:text-zinc-200 active:scale-95 transition-all cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          >
            <div className="space-y-6">
              ...
              <div className="text-center py-4">
                ...
                <div className="flex items-center justify-center gap-4 mt-2 px-10 relative">
                  ...
                  <input
                    ...
                    className={`text-center text-4xl font-extrabold tracking-tight bg-transparent border-none outline-none focus:outline-none focus:ring-0 flex-1 min-w-0 ${
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all active:scale-90 shrink-0 cursor-pointer ${
                      isCleared
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-zinc-900/50 border ${
                    type === "income" ? "border-blue-900/20 focus:border-blue-500/30" : "border-red-900/20 focus:border-red-500/30"
                  } rounded-xl py-3 px-4 text-sm text-zinc-200 outline-none transition-all placeholder:text-zinc-650`}
                />
              </div>
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-zinc-900/50 border ${
                      type === "income"
                        ? "border-blue-900/20 focus:border-blue-500/30"
                        : "border-red-900/20 focus:border-red-500/30"
                    } rounded-xl py-3 pl-4 pr-12 text-sm text-zinc-200 outline-none transition-all placeholder:text-zinc-500`}
                    required
                  />
                  <button
                    type="button"
                    onClick={() => setShowCalendarPopover(!showCalendarPopover)}
                    className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-lg text-zinc-400 hover:text-zinc-200 active:scale-95 transition-all cursor-pointer ${
                      showCalendarPopover
                        ? type === "income"
                          ? "text-blue-400"
                          : "text-red-400"
                ...
                {showCalendarPopover && (
                  <div
                    ref={calendarRef}
                    className="absolute z-30 left-0 right-0 mt-2 p-4 bg-zinc-950/95 border border-zinc-800 rounded-2xl shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-150"
                  >
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 active:scale-90 transition-all cursor-pointer"
                      >
                        <ChevronLeft className="w-4 h-4" />
                      </button>
                      <span className="text-xs font-extrabold text-zinc-200 uppercase tracking-wider select-none">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-2 text-center select-none">
                      {WEEKDAY_NAMES.map((name, i) => (
                        <span key={i} className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">
                          {name}
                        </span>
                      ))}
                    </div>
                      ...
                      {calendarDays.map((dayObj, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`aspect-square flex flex-col items-center justify-center rounded-lg text-xs transition-all relative cursor-pointer ${
                              selected
                                ? type === "income"
                                  ? "bg-blue-600 text-white font-extrabold shadow-lg shadow-blue-500/20"
                                  : "bg-red-600 text-white font-extrabold shadow-lg shadow-red-500/20"
                                : dayObj.isCurrentMonth
                                ? "text-zinc-200 hover:bg-zinc-900 hover:text-white"
                                : "text-zinc-600 hover:bg-zinc-900/30 hover:text-zinc-450"
                            }`}
                          >
                            <span>{dayObj.day}</span>
                            {today && !selected && (
                              <span className={`absolute bottom-1 w-1 h-1 rounded-full ${type === "income" ? "bg-blue-400" : "bg-red-400"}`} />
                            )}
                          </button>
                        );
                      })}
              ...
              {/* Categorias - Dropdown Customizado (sem layout nativo do sistema) */}
              <div className="relative">
                ...
                <div className="relative">
                  <button
                    ...
                    className={`w-full bg-zinc-900 border ${
                      type === "income"
                        ? "border-blue-900/30 focus:border-blue-500/30"
                        : "border-red-900/30 focus:border-red-500/30"
                    } rounded-2xl py-4 px-4 pr-10 text-sm text-zinc-200 outline-none flex items-center justify-between font-semibold active:scale-[0.98] transition-all text-left cursor-pointer`}
                  >
                    <span>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute z-20 left-0 right-0 mt-2 bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl">
                      <div className="max-h-56 overflow-y-auto divide-y divide-zinc-900/50">
                        {filteredCategories.map((cat) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left py-3.5 px-4 text-xs font-bold transition-all flex items-center justify-between active:bg-zinc-900 cursor-pointer ${
                                isSelected
                                  ? type === "income"
                ...
                <div className={recurrenceType === "single" ? "w-full" : recurrenceType === "fixed" ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "grid grid-cols-1 md:grid-cols-3 gap-6"}>
                  ...
                  {recurrenceType === "installment" && (
                    <div className="relative">
                      ...
                      <div className={`w-full bg-zinc-900 border ${type === "income" ? "border-blue-900/30" : "border-red-900/30"} rounded-2xl flex items-center justify-between overflow-hidden`}>
                        <button
                          type="button"
                          onClick={() => setInstallmentsTotal(Math.max(2, installmentsTotal - 1))}
                          className={`px-4 py-4 text-zinc-400 ${type === "income" ? "hover:text-blue-300 hover:bg-blue-950/20" : "hover:text-red-300 hover:bg-red-950/20"} transition-all font-black text-base cursor-pointer select-none`}
                        >
                          -
                        </button>
                        <input
                          ...
                          className="bg-transparent border-none text-center text-sm font-semibold text-zinc-250 outline-none w-16 focus:ring-0 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          required
                        />
                        <button
                          type="button"
            ...
            <div className="pt-4">
              <button
                ...
                className={`w-full py-4 rounded-2xl text-white font-extrabold text-sm uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-all disabled:opacity-30 disabled:pointer-events-none cursor-pointer ${
                  type === "income"
                    ? "bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-500/10"
                    : "bg-red-650 hover:bg-red-650 shadow-lg shadow-red-500/10"
                }`}
              >
