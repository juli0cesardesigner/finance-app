...
export default function Dashboard({
  ...
  const [activeTab, setActiveTab] = useState<
    "totais" | "lancamentos" | "contas" | "cartoes" | "categorias" | "configuracoes"
  >("totais");
  ...
  return (
    <div className="flex-1 flex flex-row h-full bg-black overflow-hidden min-h-0">
      <aside className="w-16 md:w-56 shrink-0 flex flex-col py-10 px-3 relative overflow-hidden my-4 ml-4 mr-2 rounded-[2rem]"
        style={{
          background: "rgba(15, 15, 20, 0.65)",
          backdropFilter: "blur(40px) saturate(210%)",
          WebkitBackdropFilter: "blur(40px) saturate(210%)",
          border: "1px solid rgba(255, 255, 255, 0.08)",
          boxShadow: "inset 1px 1px 0 rgba(255, 255, 255, 0.05), 0 8px 32px rgba(0, 0, 0, 0.6)",
        }}
      >
        {/* Apple-glass inner shimmer */}
        <div className="pointer-events-none absolute inset-0 rounded-[2rem]"
          style={{ background: "linear-gradient(180deg, rgba(255, 255, 255, 0.04) 0%, rgba(255, 255, 255, 0) 60%)" }}
        />

        {/* Seletor Global de Persona/Empresa no topo do Sidebar */}
        <div className="relative z-20 mb-4 w-full px-1">
          <button
            type="button"
            onClick={() => setIsEntityDropdownOpen(!isEntityDropdownOpen)}
            className="w-full flex items-center justify-between gap-2 px-3 py-2 bg-zinc-900/60 border border-zinc-800 hover:border-zinc-700 rounded-2xl transition-all text-left cursor-pointer"
          >
          ...
          {isEntityDropdownOpen && (
            <>
              ...
              <div className="absolute left-0 top-full mt-1.5 w-56 bg-zinc-950 border border-zinc-900 rounded-2xl shadow-2xl z-40 p-1.5 animate-fadeIn">
                <button type="button"
                  onClick={() => { onEntityChange("all"); setIsEntityDropdownOpen(false); }}
                  className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-colors flex items-center gap-2.5 ${selectedEntityId === "all" ? "bg-accent/10 text-accent font-bold" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/50"}`}
                >
                  <span className="w-5 h-5 rounded-lg bg-blue-600/10 border border-blue-500/20 text-blue-400 flex items-center justify-center"><Users className="w-3.5 h-3.5" /></span>
                  Consolidado Geral
                </button>
                ...
                {entities.filter((e) => e.type === "personal").map((e) => (
                  <button key={e.id} type="button"
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-colors flex items-center gap-2.5 ${selectedEntityId === e.id ? "bg-accent/10 text-accent font-bold" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/50"}`}
                  >
                    <span className={`w-5 h-5 rounded-lg flex items-center justify-center text-[10px] font-black ${e.id === "ent-1" ? "bg-purple-600/10 border border-purple-500/20 text-purple-400" : "bg-rose-600/10 border border-rose-500/20 text-rose-400"}`}>
                      {e.id === "ent-1" ? "J" : "H"}
                    </span>
                    {e.name}
                  </button>
                ...
                {entities.filter((e) => e.type === "business").map((e) => (
                  <button key={e.id} type="button"
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-colors flex items-center gap-2.5 ${selectedEntityId === e.id ? "bg-accent/10 text-accent font-bold" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/50"}`}
                  >
                    <span className="w-5 h-5 rounded-lg bg-amber-600/10 border border-amber-500/20 text-amber-400 flex items-center justify-center"><Building2 className="w-3.5 h-3.5" /></span>
                    {e.name}
                  </button>
                ))}
              </div>
        ...
        {/* NAVIGATION — vertically centered, pill active */}
        <nav className="flex flex-col gap-3 relative z-10 my-auto w-full">
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex-none flex items-center gap-4 px-4 h-[52px] rounded-full text-left transition-all duration-200 cursor-pointer ${
                  isActive
                    ? "bg-white text-black font-black shadow-lg shadow-white/10"
                    : "text-zinc-500 hover:text-zinc-200 font-semibold"
                }`}
              >
                <Icon className="w-[18px] h-[18px] shrink-0" />
                <span className="hidden md:inline text-[11px] tracking-wide leading-none truncate">
                  {item.label}
                </span>
              </button>
            );
      ...
      <main className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 min-h-0">
        {/* Cabeçalho de Ações */}
        <div className="flex justify-end border-b border-zinc-950 pb-5">
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => setHideValues(!hideValues)}
              className="p-2 text-zinc-550 hover:text-zinc-200 hover:bg-zinc-900/40 rounded-xl transition-all active:scale-95 flex items-center justify-center cursor-pointer"
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-red-900/30 bg-red-950/20 text-red-400 hover:bg-red-950/30 text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all active:scale-95 cursor-pointer"
            >
              <Minus className="w-3.5 h-3.5" /> Saída
            </button>
            <button
              onClick={() => onAddTransactionClick("income")}
              className="px-4 py-2.5 rounded-xl border border-blue-900/30 bg-blue-950/20 text-blue-400 hover:bg-blue-950/30 text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all active:scale-95 cursor-pointer"
            >
        ...
        {activeTab === "totais" && (
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="flex justify-center items-center py-2">
              <div className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-800 rounded-xl p-1 shadow-lg min-w-[260px] justify-between">
                <button
                  type="button"
                  onClick={handlePrevMonth}
                  className="p-2 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-lg transition-all active:scale-90 cursor-pointer"
                  title="Mês Anterior"
                >
                  <ChevronLeft className="w-4 h-4" />
                </button>
                
                <span className="text-xs font-black uppercase tracking-wider text-zinc-200 select-none text-center flex-1 px-4">
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.15em] text-zinc-400 flex items-center gap-2">
                <span className="w-1.5 h-3 bg-accent rounded-full" />
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className="text-3xl font-extrabold tracking-tight mt-2 text-emerald-400">
                        {formatMoney(monthlyIncome)}
                      </h3>
                    </div>
                    <span className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400">
                      <TrendingUp className="w-5 h-5" />
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Entradas registradas para o período de {formatMonthYear(selectedMonth)}.
                  </p>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className="text-3xl font-extrabold tracking-tight mt-2 text-rose-400">
                        {formatMoney(monthlyExpense)}
                      </h3>
                    </div>
                    <span className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-400">
                      <TrendingDown className="w-5 h-5" />
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Saídas (PIX, dinheiro, faturas) no período de {formatMonthYear(selectedMonth)}.
                  </p>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className={`text-3xl font-extrabold tracking-tight mt-2 ${monthlyBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {formatMoney(monthlyBalance)}
                      </h3>
                    </div>
                    <span className={`p-3 border rounded-2xl ${monthlyBalance >= 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}>
                      <BarChart3 className="w-5 h-5" />
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Balanço líquido das entradas vs saídas deste mês específico.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.15em] text-zinc-400 flex items-center gap-2">
                <span className="w-1.5 h-3 bg-zinc-650 rounded-full" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span className="p-3 bg-accent-dim border border-accent-border rounded-2xl text-accent">
                      <Wallet className="w-5 h-5" />
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Soma líquida em contas (PIX + Dinheiro) subtraindo faturas de Cartões.
                  </p>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className="text-3xl font-extrabold tracking-tight mt-2 text-zinc-300">
                  ...
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Soma histórica acumulada de todos os lançamentos de entrada.
                  </p>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden">
                  ...
                  <p className="text-[10px] text-zinc-550 mt-4 italic">
                    Soma histórica acumulada de todos os lançamentos de saída.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="glass p-6 rounded-3xl space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 py-2">
                <div className="flex justify-between items-center text-xs border-b border-zinc-900/40 pb-2">
                  <span className="text-zinc-500 font-medium">Lançamentos no mês selecionado</span>
                  <span className="font-bold text-white">{monthlyTransactions.length} transações</span>
                </div>
                <div className="flex justify-between items-center text-xs border-b border-zinc-900/40 pb-2">
                  <span className="text-zinc-500 font-medium">Total de lançamentos históricos</span>
                  <span className="font-bold text-white">{transactions.length} transações</span>
                </div>
                <div className="flex justify-between items-center text-xs md:border-b-0 pb-2">
                  <span className="text-zinc-500 font-medium">Balanço do mês selecionado</span>
                  <span className={`font-black ${monthlyBalance >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                    {formatMoney(monthlyBalance)}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs pb-2">
                  <span className="text-zinc-500 font-medium">Balanço geral acumulado</span>
        ...
        {activeTab === "lancamentos" && (
          <div className="space-y-4 animate-fadeIn">
            ...
            {/* Filtros e Busca */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-950/40 p-4 border border-zinc-900 rounded-2xl">
              <div className="relative w-full sm:w-[300px]">
                ...
                <input
                  ...
                  className="w-full bg-zinc-900/60 border border-zinc-800 focus:border-zinc-700 rounded-xl py-2.5 pl-10 pr-4 text-xs text-zinc-200 outline-none transition-all"
                />
              </div>

              <div className="flex bg-zinc-900/50 p-1 border border-zinc-800 rounded-xl">
                {(["all", "income", "expense"] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => setFilterType(type)}
                    className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 ${
                      filterType === type
            ...
            <div className="glass rounded-3xl overflow-hidden shadow-2xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-zinc-900 bg-zinc-950/20 text-[10px] font-black uppercase tracking-widest text-zinc-500">
                          ...
                          {sortedRegularTxs.map((t) => {
                            ...
                            return (
                              <tr key={t.id} className="hover:bg-zinc-900/10 transition-colors">
                                ...
                                <td className="py-4.5 px-6 text-zinc-200">
                                ...
                                <td className="py-4.5 px-6 text-zinc-400">{acc?.name}</td>
                                <td className={`py-4.5 px-6 text-right font-black ${isIncome ? "text-emerald-400" : "text-rose-400"}`}>
                                  {isIncome ? "+" : "-"}
                                ...
                                <td className="py-4.5 px-6 text-center">
                                  <div className="flex justify-center items-center gap-1.5">
                                    <button
                                      ...
                                      className={`w-8 h-8 rounded-xl transition-all cursor-pointer flex items-center justify-center hover:bg-zinc-900/40 ${
                                    ...
                                    <button
                                      ...
                                      className="w-8 h-8 text-zinc-550 hover:text-zinc-200 hover:bg-zinc-900/40 rounded-xl transition-all cursor-pointer flex items-center justify-center"
                                    ...
                                    {confirmDeleteId === t.id ? (
                                      <div className="flex items-center gap-1.5 animate-fadeIn">
                                        <button
                                          ...
                                          className="w-8 h-8 text-red-500 hover:text-red-400 hover:bg-red-950/20 rounded-xl transition-all cursor-pointer flex items-center justify-center"
                                        ...
                                        <button
                                          ...
                                          className="w-8 h-8 text-zinc-550 hover:text-zinc-250 hover:bg-zinc-900/40 rounded-xl transition-all cursor-pointer flex items-center justify-center"
                                          title="Cancelar"
                                        >
                                          <X className="w-4 h-4" />
                                        </button>
                                      ...
                                      <button
                                        ...
                                        className="w-8 h-8 text-zinc-550 hover:text-red-400 hover:bg-red-950/20 rounded-xl transition-all cursor-pointer flex items-center justify-center"
                            ...
                            <React.Fragment key={group.card.id}>
                              ...
                              {(() => {
                                ...
                                return (
                                  <>
                                    <tr 
                                      className="bg-zinc-900/30 border-y border-zinc-900 cursor-pointer hover:bg-zinc-900/40 select-none transition-colors"
                                      onClick={(e) => {
                                    ...
                                    >
                                      <td className="py-3 px-6 text-zinc-500 font-semibold">
                                        <div className="flex items-center gap-1.5">
                                          {isExpanded ? (
                                            <ChevronDown className="w-3.5 h-3.5 text-zinc-400 rotate-180 transition-transform duration-200" />
                                          ) : (
                                            <ChevronDown className="w-3.5 h-3.5 text-zinc-400 transition-transform duration-200" />
                                          )}
                                          <CreditCard className="w-3.5 h-3.5 text-zinc-400" />
                                          <span className="text-[10px] uppercase tracking-wider">Cartão</span>
                                        </div>
                                      </td>
                                      ...
                                      <td className="py-3 px-6">
                                        <span className="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-zinc-950/60 border border-zinc-900 text-zinc-400">
                                          Fatura Cartão
                                        </span>
                                      </td>
                                      <td className="py-3 px-6 text-zinc-550 font-semibold">{group.card.name}</td>
                                        ...
                                        <div className="flex justify-center items-center gap-1.5">
                                          {group.pendingCents > 0 ? (
                                            <button
                                              ...
                                              className="w-8 h-8 rounded-xl transition-all cursor-pointer flex items-center justify-center text-rose-400 hover:bg-emerald-950/20 hover:text-emerald-400"
                                    ...
                                    {isExpanded && group.transactions.map((t) => {
                                      ...
                                      return (
                                        <tr key={t.id} className="bg-zinc-950/20 hover:bg-zinc-900/5 transition-colors border-b border-zinc-900/40">
                                          <td className="py-3 px-6 text-zinc-550 font-medium pl-8">
                                            {new Date(t.date).toLocaleDateString("pt-BR")}
                                          </td>
                                          <td className="py-3 px-6 text-zinc-300">
                                            <div className="flex items-center gap-2 pl-2">
                                              <span className="text-zinc-700 font-bold select-none text-[10px]">↳</span>
                                          ...
                                          <td className="py-3 px-6 text-zinc-550">{group.card.name}</td>
                                          <td className={`py-3 px-6 text-right font-black ${isIncome ? "text-emerald-400" : "text-rose-400"}`}>
                                            {isIncome ? "+" : "-"}
                                            {formatMoney(t.amount_cents)}
                                          </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
            {accounts
              ...
              .map((acc) => (
                <div
                  key={acc.id}
                  className="glass p-6 rounded-3xl space-y-4 hover:scale-[1.01] transition-all relative overflow-hidden group border border-zinc-900/60"
                >
                  <div className="absolute top-0 right-0 w-20 h-20 bg-zinc-800/10 rounded-full blur-xl pointer-events-none group-hover:bg-accent/5 transition-all"></div>
                  <div className="flex justify-between items-start">
                    <div>
                    ...
                    <span className="p-2.5 rounded-xl text-xs border bg-zinc-900 border-zinc-800 text-zinc-300">
              ...
              {accounts
                ...
                .map((acc) => {
                  ...
                  return (
                    <div
                      key={acc.id}
                      className="glass p-6 rounded-3xl space-y-4 hover:scale-[1.01] transition-all relative overflow-hidden group border border-zinc-900/60 flex flex-col justify-between"
                    >
                      <div className="absolute top-0 right-0 w-20 h-20 bg-zinc-800/10 rounded-full blur-xl pointer-events-none group-hover:bg-rose-500/5 transition-all"></div>
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="p-2 rounded-xl border border-zinc-800 bg-zinc-900/60 text-zinc-600 hover:text-red-400 hover:border-red-900 hover:bg-red-950/20 active:scale-90 transition-all"
                            >
                            ...
                            <button
                              ...
                              title="Editar cartão"
                              className="p-2 rounded-xl border border-zinc-800 bg-zinc-900/60 text-zinc-400 hover:text-white hover:border-zinc-700 hover:bg-zinc-800 active:scale-90 transition-all"
                            >
                              {editingCardId === acc.id ? (
                            ...
                            <span className="p-2.5 rounded-xl text-xs border bg-rose-500/10 border-rose-500/20 text-rose-400">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[9px] font-bold text-zinc-500 border-y border-zinc-900/50 py-2.5">
                          <div>
                            <span>Fechamento:</span>
                            <span className="text-zinc-300 block">Todo dia {acc.closing_date || "3"}</span>
                          </div>
                          <div>
                            <span>Vencimento:</span>
                            <span className="text-zinc-300 block">Todo dia {acc.due_date || "10"}</span>
                          </div>
                        </div>
